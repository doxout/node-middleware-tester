...
(function (mwtest) {
    ...
    var Tester = (function () {
        ...
        Tester.prototype.request = function (opt) {
            if (!opt.url) throw new Error("URL is missing in request");
            ...
            var req = http.request(reqOptions, function (res) {
                ...
                res.pipe(bl(function (err, buf) {
                    ...
                    if (res.statusCode != expectedCode) {
                        ...
                        err = new Error("HTTP code " + res.statusCode + ' ' + msg);
                    }
                    p.callback(err, res);
                }));
                res.pipe(out);
            ...
            if (opt.body != null || opt.method == 'GET' || opt.method == 'DELETE' || opt.method == 'OPTIONS')
                req.end(postData);
            else
                inp.pipe(req);
        ...
        Tester.prototype.put = function (url, body, opt) {
            opt = opt || {};
            opt.url = url;
            opt.method = 'PUT';
            opt.body = body;
        ...
        Tester.prototype.options = function(url, opt) {
            opt = opt || {};
            opt.url = url;
            opt.method = 'OPTIONS';
            return this.request(opt);
        ...
        Tester.prototype.putJSON = function (url, body, opt) {
            opt = opt || {};
            opt.json = true;
        ...
        Tester.prototype.putJSONAsync = function (url, body, opt) {
